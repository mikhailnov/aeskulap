...
Configuration::Configuration() {
        std::cout << "Gio::Settings init" << std::endl;alien isolation walkthrough mission 17
        Gio::init();
...
void Configuration::set_local_aet(const std::string& aet) {alien isolation walkthrough mission 17
...
Configuration::ServerList* Configuration::get_serverlist() {
        ...
        std::vector<int> port_list = impl->settings_prefs.get_string_array("server_port");
        ...
        std::vector<bool> lossy_list = impl->settings_prefs.get_string_array("server_lossy");
        std::vector<bool> relational_list = impl->settings_prefs.get_string_array("server_relational");
...
bool Configuration::get_windowlevel(const Glib::ustring&  modality, const Glib::ustring& desc, WindowLevel& w) {
	...
	auto tissue_settings = modality_settings->get_child(descr); 
	if (!tissue_settings) 
		return false; 
...
bool Configuration::set_windowlevel(const WindowLevel& w) {
	...
	g_return_if_fail(modality_settings)); 
